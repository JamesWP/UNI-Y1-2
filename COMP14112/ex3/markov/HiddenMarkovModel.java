...
/**
 * HMM with normal distribution emission density for each feature vector
 * component within each state
 */

public class HiddenMarkovModel
{

  private int noStates; // Number of hidden states in the model (excluding
  // the start and stop states)
  private int featureDimension = Data.noMfcc; // We will only consider 13
  // dimensional MFCC vectors
     ...
     * transitionProbability[i][j] is probability of transition from state i
     * to state j
     */
  ...
  private double[][] transitionProbability;
    ...
    /*
     * emissionDensity [featureDimension][noStates] is a normal distribution
     * for each dimension (MFCC component) in each state
     * Probabilities are assumed to be independent for each feature,
     * similar to the naiveBayes assumption, so the probability
     * of all features is found by multiplying the probabilities of each one
     * (as done in the forward algorithm)
     */

  private Normal[][] emissionDensity;

  /**
   * Constructor method to create an HMM object with n states and
  ...
  /**
   * Constructor method to build an HMM given known transition and emission
   * probabilities
   */
  ...
  /**
   * Constructor method to train an HMM using sequence data and given the
   * corresponding data labels,
   * dataList is a list of arrays of size
   * [featureDimension][lengthSequence] storing the MFCC feature
   * vectors for each time segment in each example,
   * labelsList is a list of arrays of size [lengthSequence] storing the
   * corresponding state sequence
   */

  public HiddenMarkovModel(int n,
                           ArrayList<double[][]> dataList,
                           ArrayList<int[]> labelsList)
  {
    ...
    int[] labelSequence; // Array for storing the state path for a single
    // example
    double[][] dataSequence; // Array for storing the MFCC data for a single
    // example
    int lengthSequence;
	...
	/* 
  * For task 1 modify the code below so that the transition probabilities for
	* each state are
	...
	*/
    ...
    for (i = 0; i < noStates + 1; i++)
    {
      int totalCount = sum(transitionCount[i]);
      for (j = 0; j < noStates + 1; j++)
      {
        int myCount = transitionCount[i][j];
        transitionProbability[i][j] = (double) myCount / totalCount;
      }
    }
    ...
    for (i = 0; i < noExamples; i++)
    {
      ...
      {
        for (k = 0; k < featureDimension; k++)
          variance[k][labelSequence[j]] += Math.pow(dataSequence[k][j] -
                                                            mean[k][labelSequence[j]],
                                                    2
          );
      }
    }
  ...
  /**
   * Method for getting the emission probability distribution for a
   * particular feature and state
   */
  ...
  /**
   * Method for setting the emission probability distribution for a
   * particular feature and state
   */
   ...
   * i,j = 0 to noStates-1 for standard states, i,j = noStates for START/STOP
   * states
   */
  ...
  public BigDecimal forward(double[][] dataSequence)
  {
    ...
    MathContext mc = new MathContext(10); // Sets precision of BigDecimal
    // arithmetic used below
    BigDecimal[] f = new BigDecimal[noStates]; // Stores the quantity on the
    // left of equation (11)
    BigDecimal[] newf = new BigDecimal[noStates]; // f[] for the next time
    BigDecimal temp = new BigDecimal(0.0, mc);
    double px; // For computing the emission probabilities of all feature
    // dimensions multiplied together
    int t, i, j, k;

    // Initialisation - we have to multiply the probabilities of all the
    // feature vector components together
    ...
    for (i = 0; i < noStates; i++)
    {
      ...
      f[i] = new BigDecimal(px * transitionProbability[noStates][i],
                            mc); // Transition from first state
    }
    ...
    for (t = 1; t < sequenceLength; t++)
    {
      for (i = 0; i < noStates; i++)
      {
        ...
        {
          temp = temp.add(f[k].multiply(new BigDecimal
                                                (transitionProbability[k][i],
                                                 mc), mc), mc);
        }
    ...
    temp = new BigDecimal(0.0, mc);
    for (i = 0; i < noStates; i++)
      temp = temp.add(f[i].multiply(new BigDecimal(transitionProbability[i
                                                           ][noStates], mc),
                                    mc
      ), mc);
  ...
  /**
   * Viterbi algorithm returns the optimal path through the hidden states
   * for an input data sequence:
   * Input is dataSequence[featureDimension][sequenceLength],
   * Output is the inferred most likely path through the hidden states as an
   * array of ints
   */
  ...
  public int[] viterbi(double[][] dataSequence)
  {
	...
	/* Don't worry about the Viterbi code - it is beyond the scope of the
  course */
    ...
    double max = 0.0;
    int index = 0;
    double lpx; // Log probability of sequence up to this time and the
    // optimal path up to that state at that time
    int t, i, j, k;
    ...
    for (i = 0; i < noStates; i++)
    {
      ...
      ptr[i][0] = noStates; //Always start at the start state - we don't use
      // this element
    }
