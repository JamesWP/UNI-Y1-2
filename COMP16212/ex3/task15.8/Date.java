...
public class Date
{
  ...
  public Date addDay()
  {
    // try creating new date by next day
    try
    {
      return new Date(day+1,month,year);
    }catch (Exception e1)
    {
      // if that fails.. then try next month
      try
      {
        return new Date(1,month+1,year);
      }catch (Exception e2)
      {
        // if that fails  must be next month
  ...
  public Date addMonth()
  {
    ...
    {
      // either month doesent exist or 
      // day dosent exist
      try{
        return new Date(daysInMonth(month+1,year),month+1,year); 
      }catch (Exception e2)
      {
        // month doesent exist must be next year
  ...
  {
    // try day -1
    // if invalid then last day of prev month
    // if invalid must be last day of last month of last year
    // if invalid must be before special year
    try
    {
      return new Date(day-1,month,year);
    } catch (Exception e1)
  ...
  {
    // try day month-1 
    // try last day of month-1 month-1
    // if invalid then try day 12 year-1
    // if invalid then try last day of 12 12 year-1
    // if invalid then :(
   ...
   * is negative. It is non-negative otherwise (icluding zero
