...
int calculatePathLength(int *cameFrom, int source, int destination, int i);
...
int main(int argc, char *argv[]) {
  ...
  if(argc > 2 && strcmp(argv[2],"smallworld")==0){
    ...
    int pathLen = astar(&g,source,destination);
    if(pathLen==0){
...
int astar(Graph *g, int source, int destination) {
  ...
  while (queue_size(q)>0){
    ...
    if(u==destination){
      free(q);
      free(qid);
      return calculatePathLength(prev, source, destination, g->maxID);
    }
    List* neighbours = g->table[u]->outlist;
    while(neighbours!=NULL){
      int v = neighbours->index;
...
int calculatePathLength(int *cameFrom, int source, int destination, int maxSteps) {
  int pathIndex=1;
  ...
  while(pathIndex<maxSteps&&current!=source){
    ...
    if(current==0) {
      perror("reconstruction error encountered 0");
      exit(12);
    }
  }
  free(cameFrom);
